...
public class BookController {
	...
	@Operation(summary = "Get paginated list of books",
			description = "Returns a list of books based on pagination and sorting parameters. Accessible by all users.")
	@ApiResponses(value = { @ApiResponse(responseCode = "200", description = "List of books returned successfully") })
	...
	@Operation(summary = "Update an existing book",
			description = "Updates details of an existing book. Accessible by ADMIN role only.")
	...
	@Operation(summary = "Get books by genre",
			description = "Returns a paginated list of books belonging to a specific genre. Accessible by all users.")
	@ApiResponses(
	...
	@Operation(summary = "Get books by author",
			description = "Returns a paginated list of books by a specific author ID. Accessible by all users.")
	@ApiResponses(
			value = { @ApiResponse(responseCode = "200", description = "List of books by author returned successfully"),
					@ApiResponse(responseCode = "404", description = "Author not found") })
	public Page<BookDto> getBookByAuthor(@PathVariable Long id, @PathVariable String name, // This
																							// 'name'
																							// path
																							// variable
																							// seems
	...
	@Operation(summary = "Update book stock",
			description = "Updates the stock quantity for a specific book. Accessible by ADMIN role only.")
	...
	@Operation(summary = "Search and filter books",
			description = "Searches for books based on various criteria like title, genre, publication date range, and price range. Accessible by all users.")
	@ApiResponses(value = {
			@ApiResponse(responseCode = "200", description = "Filtered list of books returned successfully") })
	public Page<BookDto> searchBooks(@RequestParam(required = false, defaultValue = "") String search,
			@RequestParam(required = false) String genre,
			@RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate startDate,
			@RequestParam(required = false) @DateTimeFormat(iso = DateTimeFormat.ISO.DATE) LocalDate endDate,
			@RequestParam(required = false) Double minPrice, @RequestParam(required = false) Double maxPrice,
			@RequestParam(defaultValue = "title") String sortBy, @RequestParam(defaultValue = "asc") String sortOrder,
			@RequestParam(defaultValue = "0") int page, @RequestParam(defaultValue = "10") int size) {
		return bookService.getFilteredBooks(search, genre, startDate, endDate, minPrice, maxPrice, sortBy, sortOrder,
				page, size);
	}
